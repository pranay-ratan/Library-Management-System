...
# Sample Data Definition with Hashed Passwords
...
def extract_sql_to_file(sql_content, filename):
    """Extract SQL content to a file"""
    print(f"Extracting {filename} SQL file...")
    with open(filename, 'w') as f:
        f.write(sql_content)
    print(f"{filename} SQL written to {filename}")

def initialize_database(db_file, schema_sql, sample_data_sql, passwords):
    ...
    try:
        ...
        # Load sample data with hashed passwords
        print("Loading sample data...")
        cursor.executescript(sample_data_sql.replace('?', "'{}'").format(*passwords))
...
def main():
    # Extract SQL to files
    extract_sql_to_file(SCHEMA_SQL, SCHEMA_FILE)
    extract_sql_to_file(SAMPLE_DATA_SQL, SAMPLE_DATA_FILE)
    
    # Hash passwords for sample data
    default_password = "password123"  # Default password for all users
    hashed_password = hash_password(default_password)
    passwords = [hashed_password] * 27  # Adjust number based on total Member + Staff records
    
    # Initialize the database
    initialize_database(DB_FILE, SCHEMA_SQL, SAMPLE_DATA_SQL, passwords)
