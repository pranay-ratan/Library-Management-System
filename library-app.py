...
class LibrarySystem:
    ...
    def login(self):
        ...
        if choice == '1':
            ...
            password = getpass("Enter your password: ")  # Add password for staff
            ...
            if result and len(result) > 0:
                user = dict(result[0])
                # Assuming Staff table now has a Password column
    ...
    def borrow_item(self, item_id=None):
        ...
        if not item_id:
            item_id = input("Enter the ID of the item you want to borrow (or 'cancel' to return): ")
    ...
    def reserve_item(self):
        if self.user_type != "member":
            print("\nYou need to be logged in as a member to reserve items.")
            input("Press Enter to continue...")
            return
        ...
        item_id = input("Enter the ID of the item you want to reserve (or 'cancel' to return): ")
        if item_id.lower() == 'cancel':
            return
        if not item_id.isdigit():
            print("\nInvalid item ID. Please enter a number.")
        ...
        if item['Status'] == 'Available':
            print(f"\nItem '{item['Title']}' is currently available. Would you like to borrow it instead?")
            choice = input("(y/n): ")
            if choice.lower() == 'y':
                self.borrow_item(item_id)
            return
        ...
        query = "SELECT * FROM Reservation WHERE MemberID = ? AND ItemID = ? AND Status = 'Pending'"
        existing = self.execute_query(query, (self.current_user['MemberID'], item_id))
        
        if existing and len(existing) > 0:
            print(f"\nYou already have a pending reservation for '{item['Title']}'.")
            input("Press Enter to continue...")
            return
        
        # Create reservation
        query = "INSERT INTO Reservation (MemberID, ItemID) VALUES (?, ?)"
        result = self.execute_query(query, (self.current_user['MemberID'], item_id), fetch=False, commit=True)
        
        if result:
            print(f"\nSuccessfully reserved '{item['Title']}'. You will be notified when it becomes available.")
        else:
    ...
    def member_menu(self):
        while True:
            ...
            print("10. View My Account")
            print("11. Pay Fines")
            print("12. Cancel Event Registration")
            print("13. Log Out")
            ...
            if choice == '1':
                ...
                self.cancel_event_registration()
            elif choice == '13':
                self.current_user = None
                self.user_type = None
                print("\nYou have been logged out.")
    ...
    def cancel_event_registration(self):
        if self.user_type != "member":
            print("\nYou need to be logged in as a member to cancel event registrations.")
            input("Press Enter to continue...")
            return
        ...
        query = """
        SELECT e.EventID, e.Title, e.EventDate, e.StartTime, e.EndTime, r.RoomName
        ...
        ORDER BY e.EventDate, e.StartTime
        """
        registrations = self.execute_query(query, (self.current_user['MemberID'],))
        
        if not registrations or len(registrations) == 0:
            print("\nYou have no upcoming event registrations to cancel.")
            input("Press Enter to continue...")
            return
            
        headers = ["Event ID", "Title", "Date", "Time", "Location"]
        table_data = []
        ...
        # Verify registration
        reg = next((r for r in registrations if r['EventID'] == event_id), None)
        if not reg:
            print(f"\nEvent ID {event_id} not found in your registrations.")
            input("Press Enter to continue...")
            return
        ...
        WHERE EventID = ? AND MemberID = ?
        """
        result = self.execute_query(query, (event_id, self.current_user['MemberID']), fetch=False, commit=True)
        
        if result:
            print(f"\nRegistration for '{reg['Title']}' has been cancelled.")
        else:
    ...
    def pay_fine(self):
        if self.user_type != "member":
            print("\nYou need to be logged in as a member to pay fines.")
            input("Press Enter to continue...")
            return
        ...
        # Get unpaid fines
        query = """
        SELECT f.FineID, f.Amount, f.IssuedDate, i.Title, b.DueDate, b.ReturnDate
        FROM Fine f
        ...
        print("\nYour Unpaid Fines:")
        print(tabulate(table_data, headers=headers, tablefmt="grid"))
        print(f"Total unpaid: ${total_fines:.2f}")
        
        fine_id = input("\nEnter Fine ID to pay (or 'all' to pay all, '0' to cancel): ")
        
        if fine_id == '0':
            return
        
        if fine_id.lower() == 'all':
            query = """
            UPDATE Fine
            SET Status = 'Paid', PaidDate = ?
            WHERE BorrowID IN (
                SELECT BorrowID FROM Borrowing WHERE MemberID = ?
            ) AND Status = 'Unpaid'
            """
            result = self.execute_query(query, (datetime.date.today(), self.current_user['MemberID']), fetch=False, commit=True)
            if result:
                print(f"\nAll fines totaling ${total_fines:.2f} have been paid.")
            else:
            ...
            # Verify fine belongs to member
            fine = next((f for f in fines if f['FineID'] == fine_id), None)
            if not fine:
                print(f"\nFine ID {fine_id} not found in your unpaid fines.")
                input("Press Enter to continue...")
                return
            
            query = "UPDATE Fine SET Status = 'Paid', PaidDate = ? WHERE FineID = ?"
            result = self.execute_query(query, (datetime.date.today(), fine_id), fetch=False, commit=True)
            if result:
                print(f"\nFine of ${fine['Amount']:.2f} has been paid.")
            else:
                print("\nFailed to process payment. Please try again.")
